...
async def parse_stats(name, tag, headers, num_matches=3):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"https://pd.eu.a.pvp.net/mmr/v1/players/{user_id}/competitiveupdates?startIndex=21&endIndex=30",
                headers=headers,
            ) as r:
                data = json.loads(await r.text())
            ...
            async with session.get(
                f"https://pd.eu.a.pvp.net/match-history/v1/history/{user_id}?startIndex=20&endIndex=40&queue=competitive",
                headers=headers,
            ) as r:
                data = json.loads(await r.text())
